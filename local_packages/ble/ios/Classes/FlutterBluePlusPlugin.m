...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  FlutterMethodChannel *methodChannel =
      [FlutterMethodChannel methodChannelWithName:NAMESPACE @"/methods"
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  @try {
    ...
    if (self.centralManager == nil) {
      ...
      if ([self.restoreState boolValue]) {
        options[CBCentralManagerOptionRestoreIdentifierKey] =
            @"flutterBluePlusRestoreIdentifier";
      }

      Log(LDEBUG, @"showPowerAlert: %@",
          [self.showPowerAlert boolValue] ? @"yes" : @"no");
      Log(LDEBUG, @"restoreState: %@",
    ...
    if (self.checkForMtuChangesTimer == nil) {
      ...
      self.checkForMtuChangesTimer = [NSTimer
          scheduledTimerWithTimeInterval:0.025
                                  target:self
    ...
    if ([@"flutterRestart" isEqualToString:call.method]) {
      ...
      Log(LDEBUG, @"connectedPeripherals: %lu",
    ...
    if ([@"getAdapterState" isEqualToString:call.method]) {
      ...
      if ([self isAdapterOn] == false) {
        ...
        NSString *s = [NSString
            stringWithFormat:@"bluetooth must be turned on. (%@)", as];
        result([FlutterError errorWithCode:@"startScan"
                                   message:s
                                   details:NULL]);
        return;
      }
      ...
      if ([continuousUpdates boolValue]) {
        [scanOpts setObject:[NSNumber numberWithBool:YES]
      ...
      BOOL hasCustomFilters =
          ...
          [self hasFilter:@"with_names"] || [self hasFilter:@"with_keywords"] ||
          [self hasFilter:@"with_msd"] || [self hasFilter:@"with_service_data"];
      ...
      NSArray *periphs = [self.centralManager
          retrieveConnectedPeripheralsWithServices:withServices];
      ...
      // to it. Why? CoreBluetooth does not keep strong references and will warn
      // about API MISUSE and weak ptrs.
      ...
      if (@available(iOS 17, *)) {
        ...
        [options setObject:autoConnect
                    forKey:@"kCBConnectOptionEnableAutoReconnect"];
      }

      [self.centralManager connectPeripheral:peripheral options:options];

      // add to currently connecting peripherals
      [self.currentlyConnectingPeripherals setObject:peripheral
                                              forKey:remoteId];
      ...
      if (peripheral == nil) {
        peripheral =
            [self.currentlyConnectingPeripherals objectForKey:remoteId];
        if (peripheral != nil) {
      ...
      if (peripheral == nil) {
        NSString *s = @"device is disconnected";
        result([FlutterError errorWithCode:@"readCharacteristic"
                                   message:s
                                   details:remoteId]);
        return;
      }
      ...
      CBCharacteristic *characteristic =
          [self locateCharacteristic:characteristicUuid
      ...
      if ((characteristic.properties & CBCharacteristicPropertyRead) == 0) {
        ...
        result([FlutterError errorWithCode:@"writeCharacteristic"
                                   message:s
                                   details:NULL]);
        return;
      }
      ...
      int maxLen = [self getMaxPayload:peripheral
                               forType:writeType
                        allowLongWrite:[allowLongWrite boolValue]];
      int dataLen = (int)[self convertHexToData:value].length;
      if (dataLen > maxLen) {
        NSString *t = [writeTypeNumber intValue] == 0 ? @"withResponse"
                                                      : @"withoutResponse";
        NSString *a =
            [allowLongWrite boolValue] ? @", allowLongWrite" : @", noLongWrite";
      ...
      if (writeType == CBCharacteristicWriteWithoutResponse &&
        ...
        NSString *s =
            @"canSendWriteWithoutResponse is false. you must slow down";
        result([FlutterError errorWithCode:@"writeCharacteristic"
                                   message:s
                                   details:NULL]);
        return;
      ...
      if (writeType == CBCharacteristicWriteWithoutResponse) {
        if ((characteristic.properties &
          ...
          NSString *s = @"The WRITE_NO_RESPONSE property is not supported by "
                        @"this BLE characteristic";
          result([FlutterError errorWithCode:@"writeCharacteristic"
                                     message:s
                                     details:NULL]);
          return;
      ...
      NSString *key = [NSString
          stringWithFormat:@"%@:%@:%@:%@", remoteId, serviceUuid,
                           characteristicUuid,
      ...
      if (peripheral == nil) {
        NSString *s = @"device is disconnected";
        result([FlutterError errorWithCode:@"readDescriptor"
                                   message:s
                                   details:remoteId]);
        return;
      }
      ...
      // Find descriptor
      CBDescriptor *descriptor = [self locateDescriptor:descriptorUuid
      ...
      if (peripheral == nil) {
        NSString *s = @"device is disconnected";
        result([FlutterError errorWithCode:@"writeDescriptor"
                                   message:s
                                   details:remoteId]);
        return;
      }
      ...
      if ((mtu - 3) < dataLen) {
        NSString *f =
            @"data is longer than MTU allows. dataLen: %d > maxDataLen: %d";
      ...
      if (peripheral == nil) {
        NSString *s = @"device is disconnected";
        result([FlutterError errorWithCode:@"setNotifyValue"
                                   message:s
                                   details:remoteId]);
        return;
      }
      ...
      if (!canIndicate && !canNotify) {
        NSString *s = @"neither NOTIFY nor INDICATE properties are supported "
                      @"by this BLE characteristic";
        result([FlutterError errorWithCode:@"setNotifyValue"
                                   message:s
                                   details:NULL]);
        return;
      ...
      CBDescriptor *descriptor = [self locateDescriptor:CCCD
                                         characteristic:characteristic
                                                  error:nil];
      if (descriptor == nil) {
        Log(LWARNING,
            @"Warning: CCCD descriptor for characteristic not found: %@",
            characteristicUuid);
      }

      // Set notification value
      [peripheral setNotifyValue:[enable boolValue]
               forCharacteristic:characteristic];

      result(@YES);
    } else if ([@"requestMtu" isEqualToString:call.method]) {
      result([FlutterError
          errorWithCode:@"requestMtu"
      ...
      if (peripheral == nil) {
        NSString *s = @"device is disconnected";
        result([FlutterError errorWithCode:@"readRssi"
                                   message:s
                                   details:remoteId]);
        return;
      }
    ...
    result([FlutterError errorWithCode:@"iosException"
                               message:[e reason]
                               details:details]);
  }
}
...
- (CBCharacteristic *)locateCharacteristic:(NSString *)characteristicId
  ...
  // primary service
  CBService *primaryService = [self getServiceFromArray:primaryServiceUuid
                                                  array:[peripheral services]];
  if (primaryService == nil || !primaryService.isPrimary) {
    NSString *s = [NSString
        stringWithFormat:@"primary service not found '%@'", primaryServiceUuid];
  ...
  if (isSecondaryService) {
    secondaryService =
        [self getServiceFromArray:serviceUuid
                            array:[primaryService includedServices]];
    if (error && !secondaryService) {
      NSString *s = [NSString
          stringWithFormat:
              @"secondary service not found '%@' (primary service %@)",
              serviceUuid, primaryServiceUuid];
      NSDictionary *d = @{NSLocalizedDescriptionKey : s};
      *error = [NSError errorWithDomain:@"flutterBluePlus"
                                   code:1001
                               userInfo:d];
      return nil;
    }
  ...
  // which service?
  CBService *service =
      (secondaryService != nil) ? secondaryService : primaryService;

  // characteristic
  CBCharacteristic *characteristic =
      [self getCharacteristicFromArray:characteristicId
                                 array:[service characteristics]];
  if (characteristic == nil) {
    NSString *format =
        @"characteristic not found in service (chr: '%@', svc: '%@')";
    NSString *s =
        [NSString stringWithFormat:format, characteristicId, serviceUuid];
    NSDictionary *d = @{NSLocalizedDescriptionKey : s};
...
- (CBDescriptor *)locateDescriptor:(NSString *)descriptorId
                    characteristic:(CBCharacteristic *)characteristic
                             error:(NSError **)error {
  CBDescriptor *descriptor =
      [self getDescriptorFromArray:descriptorId
                             array:[characteristic descriptors]];
  if (descriptor == nil && error != nil) {
    NSString *format =
        @"descriptor not found in characteristic (desc: '%@', chr: '%@')";
    NSString *s = [NSString
        stringWithFormat:format, descriptorId, [characteristic.UUID uuidStr]];
    NSDictionary *d = @{NSLocalizedDescriptionKey : s};
    *error = [NSError errorWithDomain:@"flutterBluePlus" code:1002 userInfo:d];
    return nil;
  }
...
- (CBService *)getServiceFromArray:(NSString *)uuid
                             array:(NSArray<CBService *> *)array {
...
- (CBCharacteristic *)getCharacteristicFromArray:(NSString *)uuid
                                           array:(NSArray<CBCharacteristic *> *)
                                                     array {
...
- (CBDescriptor *)getDescriptorFromArray:(NSString *)uuid
                                   array:(NSArray<CBDescriptor *> *)array {
  ...
  for (NSString *key in self.connectedPeripherals) {
    ...
    if ([func isEqualToString:@"adapterTurnOff"]) {
      ...
      NSDictionary *result = @{
        ...
        @"connection_state" :
            @([self bmConnectionStateEnum:CBPeripheralStateDisconnected]),
        @"disconnect_reason_code" :
            @(1573878), // just a random value, could be anything.
        @"disconnect_reason_string" : @"Bluetooth turned off",
      };

      // Send connection state
      [self.methodChannel invokeMethod:@"OnConnectionStateChanged"
                             arguments:result];
    }
...
- (void)checkForMtuChangesCallback {
  for (NSString *key in self.connectedPeripherals) {
    ...
    NSNumber *prevMtu =
        (NSNumber *)[self.peripheralMtu objectForKey:peripheral];
...
- (void)centralManager:(CBCentralManager *)central
  ...
  for (CBPeripheral *peripheral in peripherals) {
    ...
    if (peripheral.state != CBPeripheralStateConnected) {
      // connect
      Log(LDEBUG, @"Restore: reconnecting to %@",
          peripheral.identifier.UUIDString);
      [self.centralManager connectPeripheral:peripheral options:nil];
    } else {
      // update connection state
      Log(LDEBUG, @"Restore: already connected to %@",
          peripheral.identifier.UUIDString);
...
- (void)centralManager:(CBCentralManager *)central
  ...
  // are any filters set?
  BOOL isAnyFilterSet =
      [self hasFilter:@"with_services"] ||
      [self hasFilter:@"with_remote_ids"] || [self hasFilter:@"with_names"] ||
      [self hasFilter:@"with_keywords"] || [self hasFilter:@"with_msd"] ||
  ...
  if (isAnyFilterSet) {
    // filter services
    if (!allow && [self foundService:self.scanFilters[@"with_services"]
                              target:advServices]) {
      allow = YES;
    }

    // filter remoteIds
    if (!allow && [self foundRemoteId:self.scanFilters[@"with_remote_ids"]
                               target:remoteId]) {
      allow = YES;
    }

    // filter names
    if (!allow && [self foundName:self.scanFilters[@"with_names"]
                           target:advName]) {
      allow = YES;
    }

    // filter keywords
    if (!allow && [self foundKeyword:self.scanFilters[@"with_keywords"]
                              target:advName]) {
      allow = YES;
    }
    ...
    // filter service data
    if (!allow && [self foundServiceData:self.scanFilters[@"with_service_data"]
                                      sd:advSd]) {
      allow = YES;
    }
  }
  ...
  // See BmScanResponse
  NSDictionary *response = @{
    @"advertisements" : @[ [self bmScanAdvertisement:remoteId
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  if (self.servicesToDiscover.count > 0 ||
      self.characteristicsToDiscover.count > 0) {
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  CBService *primaryService = [self getPrimaryService:peripheral
                                       characteristic:characteristic];
  ...
  // remove if null
  if (!primaryService) {
    [result removeObjectForKey:@"primary_service_uuid"];
  }

  [self.methodChannel invokeMethod:@"OnCharacteristicReceived"
                         arguments:result];
}
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  NSString *key = [NSString stringWithFormat:@"%@:%@:%@:%@", remoteId,
                                             serviceUuid, characteristicUuid,
                                             primaryService != nil
                                                 ? [primaryService.UUID uuidStr]
                                                 : @""];
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  // Oddly iOS does not update the CCCD descriptors when
  // didUpdateNotificationState is called. So instead of using
  // characteristic.descriptors we have to manually recreate the CCCD descriptor
  // using isNotifying & characteristic.properties
  int value = 0;
  if (characteristic.isNotifying) {
    ...
    bool supportsIndicate =
        (characteristic.properties & CBCharacteristicPropertyIndicate) != 0;
    if (characteristic.isNotifying && supportsIndicate) {
      value = 2;
    } // '2' comes from the CCCD BLE spec
    if (characteristic.isNotifying && supportsNotify) {
      value = 1;
    } // '1' comes from the CCCD BLE spec
  }
  ...
  NSMutableDictionary *result = [@{
    ...
    @"primary_service_uuid" : primaryService ? [primaryService.UUID uuidStr]
                                             : [NSNull null],
    @"value" : [self convertDataToHex:[NSData dataWithBytes:&value
                                                     length:sizeof(value)]],
    @"success" : @(error == nil),
    @"error_string" : error ? [error localizedDescription] : @"success",
    @"error_code" : error ? @(error.code) : @(0),
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  CBService *primaryService =
      [self getPrimaryService:peripheral
...
- (void)peripheralIsReadyToSendWriteWithoutResponse:(CBPeripheral *)peripheral {
  ...
  NSDictionary *request = [self.didWriteWithoutResponse
      objectForKey:[[peripheral identifier] UUIDString]];
  if (request == nil) {
  ...
  NSMutableDictionary *result = [@{
    ...
    @"service_uuid" : [characteristic.service.UUID uuidStr],
    @"characteristic_uuid" : [characteristic.UUID uuidStr],
    @"primary_service_uuid" : primaryServiceUuid != nil ? primaryServiceUuid
                                                        : [NSNull null],
    @"value" : value,
    @"success" : @(error == nil),
    @"error_string" : error ? [error localizedDescription] : @"success",
...
- (NSDictionary *)bmScanAdvertisement:(NSString *)remoteId
  ...
  if (manufData != nil && manufData.length >= 2) {
    ...
    NSData *trimmed =
        [manufData subdataWithRange:NSMakeRange(2, manufData.length - 2)];
  ...
  if (advName) {
    map[@"adv_name"] = advName;
  }
  if (connectable.boolValue) {
    map[@"connectable"] = connectable;
  }
  if (txPower) {
    map[@"tx_power_level"] = txPower;
  }
  if (manufDataB) {
    map[@"manufacturer_data"] = manufDataB;
  }
  if (serviceUuidsB) {
    map[@"service_uuids"] = serviceUuidsB;
  }
  if (serviceDataB) {
    map[@"service_data"] = serviceDataB;
  }
  if (RSSI) {
    map[@"rssi"] = RSSI;
  }
...
- (NSDictionary *)bmBluetoothService:(CBPeripheral *)peripheral
  ...
  for (CBCharacteristic *c in [service characteristics]) {
    [characteristicProtos addObject:[self bmBluetoothCharacteristic:peripheral
                                                     characteristic:c]];
  }
  ...
  for (CBService *included in [service includedServices]) {
    ...
    [includedServicesProtos addObject:[self bmBluetoothService:peripheral
                                                       service:included]];
  }
...
- (NSDictionary *)bmBluetoothCharacteristic:(CBPeripheral *)peripheral
  ...
  for (CBDescriptor *d in [characteristic descriptors]) {
    ...
    // remove if null
    if (!primaryService) {
      [desc removeObjectForKey:@"primary_service_uuid"];
    }
  ...
  NSDictionary *propsMap = @{
    ...
    @"extended_properties" :
        @((props & CBCharacteristicPropertyExtendedProperties) != 0),
...
- (NSInteger)scanCountIncrement:(NSString *)remoteId {
  if (!self.scanCounts[remoteId]) {
    self.scanCounts[remoteId] = @(0);
  }
...
- (BOOL)foundKeyword:(NSArray<NSString *> *)keywords target:(NSString *)target {
  if (target == nil) {
    return NO;
  }
...
- (BOOL)foundName:(NSArray<NSString *> *)names target:(NSString *)target {
  if (target == nil) {
    return NO;
  }
...
- (BOOL)foundServiceData:(NSArray<NSDictionary *> *)filters
  ...
  for (NSDictionary *f in filters) {
    ...
    if (mask.length == 0 && data.length > 0) {
      ...
      mask = [NSData dataWithBytesNoCopy:bytes
                                  length:data.length
                            freeWhenDone:YES];
    }
    ...
    for (CBUUID *uuid in sd) {
      ...
      if ([a isEqualToString:b] && [self findData:data
                                           inData:sd[uuid]
                                        usingMask:mask]) {
        return YES;
      }
...
- (BOOL)foundMsd:(NSArray<NSDictionary *> *)filters msd:(NSData *)msd {
  ...
  for (NSDictionary *f in filters) {
    ...
    if (mId == [manufacturerId integerValue] && [self findData:data
                                                        inData:trim
                                                     usingMask:mask]) {
      return YES;
    }
...
- (NSData *)convertHexToData:(NSString *)hexString {
  ...
  for (NSInteger i = 0; i < hexString.length; i += 2) {
    ...
    [[NSScanner scannerWithString:[hexString substringWithRange:range]]
        scanHexInt:&byte];
...
- (int)getMaxPayload:(CBPeripheral *)peripheral
             forType:(CBCharacteristicWriteType)writeType
      allowLongWrite:(bool)allowLongWrite {
...
- (int)getMtu:(CBPeripheral *)peripheral {
  int maxPayload = [self getMaxPayload:peripheral
