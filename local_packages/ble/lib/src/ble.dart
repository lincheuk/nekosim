...
class Ble {
  ...
  static final _scanResults =
      _StreamControllerReEmit<List<ScanResult>>(initialValue: []);
  ...
  static Future<bool> get isSupported async {
    if (Platform.isWindows) {
      return await BleWindows.isSupported;
    }
    return await _invokeMethod('isSupported');
  }
  ...
  static Future<String> get adapterName async {
    if (Platform.isWindows) {
      return await BleWindows.adapterName;
    }
    return await _invokeMethod('getAdapterName');
  }

  /// returns whether we are scanning as a stream
  static Stream<bool> get isScanning {
    if (Platform.isWindows) {
      return BleWindows.isScanning;
    }
  ...
  static bool get isScanningNow {
    if (Platform.isWindows) {
      return BleWindows.isScanningNow;
    }
  ...
  static Future<void> setOptions({
    ...
    await _invokeMethod('setOptions',
        {"show_power_alert": showPowerAlert, "restore_state": restoreState});
  }

  /// Turn on Bluetooth (Android only),
  static Future<void> turnOn({int timeout = 60}) async {
    if (Platform.isWindows) {
      return await BleWindows.turnOn(timeout: timeout);
    }
    ...
    if (changed) {
      ...
      if (response.userAccepted == false) {
        throw BleException(ErrorPlatform.fbp, "turnOn",
  ...
  static Future<List<BluetoothDevice>> systemDevices(
    ...
    return r.devices
        .map((d) => BluetoothDevice.fromId(d.remoteId.str))
        .toList();
  }
  ...
  static Future<void> startScan({
    ...
    if (Platform.isWindows) {
      return await BleWindows.startScan(
    ...
    assert(removeIfGone == null || !oneByOne,
        "removeIfGone is not compatible with oneByOne");
    ...
    try {
      ...
      Stream<BmScanResponse> responseStream = Ble._methodStream.stream
  ...
  static Future<void> stopScan() async {
    if (Platform.isWindows) {
      return await BleWindows.stopScan();
    }
    _Mutex mtx = _MutexFactory.getMutexForKey("scan");
    await mtx.take();
    try {
  ...
  static void cancelWhenScanComplete(StreamSubscription subscription) {
    if (Platform.isWindows) {
      return BleWindows.cancelWhenScanComplete(subscription);
    }
    Ble._scanSubscriptions.add(subscription);
  }

  /// Sets the internal FlutterBlue log level
  static Future<void> setLogLevel(LogLevel level, {color = true}) async {
    if (Platform.isWindows) {
      return BleWindows.setLogLevel(level, color: color);
    }
    _logLevel = level;
    _logColor = color;
  ...
  static Future<PhySupport> getPhySupport() async {
    ...
    if (!kIsWeb && Platform.isAndroid == false) {
      throw BleException(ErrorPlatform.fbp, "getPhySupport",
          FbpErrorCode.androidOnly.index, "android-only");
    }

    return await _invokeMethod('getPhySupport')
        .then((args) => PhySupport.fromMap(args));
  }
  ...
  static Future<dynamic> _methodCallHandler(MethodCall call) async {
    ...
    if (call.method == "OnNameChanged") {
      ...
      if (!kIsWeb &&
          (!kIsWeb && Platform.isMacOS || !kIsWeb && Platform.isIOS)) {
    ...
    if (call.method == "OnCharacteristicReceived" ||
        call.method == "OnCharacteristicWritten") {
      var r = BmCharacteristicData.fromMap(call.arguments);
      if (r.success == true) {
        _lastChrs[r.remoteId] ??= {};
        _lastChrs[r.remoteId]!["${r.serviceUuid}:${r.characteristicUuid}"] =
            r.value;
      }
    }
    ...
    if (call.method == "OnDescriptorRead" ||
        call.method == "OnDescriptorWritten") {
      var r = BmDescriptorData.fromMap(call.arguments);
      if (r.success == true) {
        _lastDescs[r.remoteId] ??= {};
        _lastDescs[r.remoteId]![
                "${r.serviceUuid}:${r.characteristicUuid}:${r.descriptorUuid}"] =
            r.value;
      }
    }
...
class MsdFilter {
  ...
  // convert to bmMsg
  BmMsdFilter get _bm {
    assert(mask.isEmpty || (data.length == mask.length),
        "mask & data must be same length");
...
class BleException implements Exception {
  ...
  BleException(this.platform, this.function, this.code, this.description);

  @override
  String toString() {
    String sPlatform = platform.toString().split('.').last;
    return 'BleException | $function | $sPlatform-code: $code | $description';
  }
