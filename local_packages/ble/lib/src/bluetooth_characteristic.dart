...
class BluetoothCharacteristic {
  ...
  List<int> get lastValue {
    ...
    return Ble._lastChrs[remoteId]?[key] ?? [];
  }
  ...
  Stream<List<int>> get lastValueStream => Ble._methodStream.stream
      .where((m) =>
  ...
  ///   - anytime `read()` is called
  ///   - anytime a notification arrives (if subscribed)
  Stream<List<int>> get onValueReceived => Ble._methodStream.stream
  ...
  Future<List<int>> read({int timeout = 15}) async {
    if (Platform.isWindows) {
      return await BluetoothCharacteristicWindows(
        remoteId: remoteId,
        serviceUuid: serviceUuid,
        characteristicUuid: characteristicUuid,
        descriptors: descriptors,
        propertiesWinBle: properties.toWinBle(), // I'll need to add this helper
      ).read(timeout: timeout);
    }
    // check connected
    if (device.isDisconnected) {
      throw BleException(ErrorPlatform.fbp, "readCharacteristic",
    ...
    try {
      ...
      // invoke
      await Ble._invokeMethod('readCharacteristic', request.toMap());
      ...
      // failed?
      if (!response.success) {
        throw BleException(_nativeError, "readCharacteristic",
  ...
  Future<void> write(List<int> value,
    ...
    if (Platform.isWindows) {
      return await BluetoothCharacteristicWindows(
        ...
        characteristicUuid: characteristicUuid,
        descriptors: descriptors,
        propertiesWinBle: properties.toWinBle(),
      ).write(value,
    ...
    // check connected
    if (device.isDisconnected) {
      throw BleException(ErrorPlatform.fbp, "writeCharacteristic",
          FbpErrorCode.deviceIsDisconnected.index, "device is not connected");
    }
    ...
    try {
      ...
      // invoke
      await Ble._invokeMethod('writeCharacteristic', request.toMap());
      ...
      // failed?
      if (!response.success) {
        // Some devices report this for harmless write-without-response paths.
        if (response.errorString.contains("Writing is not permitted") ||
            response.errorString.contains("GATT_WRITE_NOT_PERMITTED")) {
          // ignore
        } else {
          throw BleException(_nativeError, "writeCharacteristic",
              response.errorCode, response.errorString);
        }
      }
  ...
  Future<bool> setNotifyValue(bool notify,
    ...
    if (Platform.isWindows) {
      return await BluetoothCharacteristicWindows(
        ...
        propertiesWinBle: properties.toWinBle(),
          ...
          timeout: timeout, forceIndications: forceIndications);
    }
    // check connected
    if (device.isDisconnected) {
      throw BleException(ErrorPlatform.fbp, "setNotifyValue",
          FbpErrorCode.deviceIsDisconnected.index, "device is not connected");
    }
    ...
    try {
      ...
      Stream<BmDescriptorData> responseStream = Ble._methodStream.stream
      ...
      // invoke
      bool hasCCCD = await Ble._invokeMethod('setNotifyValue', request.toMap());
      ...
      if (hasCCCD) {
        ...
        // failed?
        if (!response.success) {
          // Some devices report this for harmless notification setup paths.
          if (response.errorString.contains("Writing is not permitted") ||
              response.errorString.contains("GATT_WRITE_NOT_PERMITTED")) {
            // ignore
          } else {
            throw BleException(_nativeError, "setNotifyValue",
                response.errorCode, response.errorString);
          }
        }
      }
  ...
  BmBluetoothService? get _bmsvc {
    if (Ble._knownServices[remoteId] != null) {
      for (var s in Ble._knownServices[remoteId]!.services) {
