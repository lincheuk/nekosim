...
class BluetoothDescriptor {
  ...
  List<int> get lastValue {
    ...
    return Ble._lastDescs[remoteId]?[key] ?? [];
  }
  ...
  Future<List<int>> read({int timeout = 15}) async {
    ...
    if (device.isDisconnected) {
      throw BleException(ErrorPlatform.fbp, "readDescriptor",
    ...
    try {
      ...
      // invoke
      await Ble._invokeMethod('readDescriptor', request.toMap());
      ...
      // failed?
      if (!response.success) {
        throw BleException(_nativeError, "readDescriptor", response.errorCode,
  ...
  Future<void> write(List<int> value, {int timeout = 15}) async {
    // check connected
    if (device.isDisconnected) {
      throw BleException(ErrorPlatform.fbp, "writeDescriptor",
          FbpErrorCode.deviceIsDisconnected.index, "device is not connected");
    }
    ...
    try {
      ...
      // invoke
      await Ble._invokeMethod('writeDescriptor', request.toMap());
      ...
      // failed?
      if (!response.success) {
        throw BleException(_nativeError, "writeDescriptor", response.errorCode,
            response.errorString);
      }
    } finally {
      mtx.give();
