...
class BluetoothDevice {
  ...
  factory BluetoothDevice.fromId(String remoteId) {
    return BluetoothDevice(remoteId: DeviceIdentifier(remoteId));
  }
  ...
  String get platformName => Ble._platformNames[remoteId] ?? "";
  ...
  String get advName => Ble._advNames[remoteId] ?? "";
  ...
  List<BluetoothService> get servicesList {
    BmDiscoverServicesResult? result = Ble._knownServices[remoteId];
    if (result == null) {
      return [];
    } else {
  ...
  void cancelWhenDisconnected(StreamSubscription subscription,
    ...
    if (isConnected == false && next == false) {
      ...
      Ble._delayedSubscriptions[remoteId] ??= [];
      Ble._delayedSubscriptions[remoteId]!.add(subscription);
    } else {
      Ble._deviceSubscriptions[remoteId] ??= [];
      Ble._deviceSubscriptions[remoteId]!.add(subscription);
    }
  }

  /// Returns true if autoConnect is currently enabled for this device
  bool get isAutoConnectEnabled {
    return Ble._autoConnect.contains(remoteId);
  }

  /// Returns true if this device is currently connected to your app
  bool get isConnected {
    if (Ble._connectionStates[remoteId] == null) {
      return false;
    } else {
      var state = Ble._connectionStates[remoteId]!.connectionState;
  ...
  Future<void> connect({
    ...
    if (Platform.isWindows) {
      return await BluetoothDeviceWindows(remoteId: remoteId).connect(
        timeout: timeout,
        mtu: mtu,
    ...
    try {
      ...
      var responseStream = Ble._methodStream.stream
      ...
      if (!kIsWeb && Platform.isAndroid) {
        Ble._connectTimestamp[remoteId] = DateTime.now();
      }

      // invoke
      bool changed = await Ble._invokeMethod('connect', request.toMap());
      ...
      if (changed && !autoConnect) {
        BmConnectionStateResponse response = await futureState
            ...
            .catchError((e) async {
          if (e is BleException && e.code == FbpErrorCode.timeout.index) {
            print("[FBP] connection timeout");
            await Ble._invokeMethod(
        ...
        if (response.connectionState == BmConnectionStateEnum.disconnected) {
          if (response.disconnectReasonCode == bmUserCanceledErrorCode) {
            throw BleException(ErrorPlatform.fbp, "connect",
                FbpErrorCode.connectionCanceled.index, "connection canceled");
          } else {
            throw BleException(_nativeError, "connect",
  ...
  Future<void> disconnect({
    ...
    if (Platform.isWindows) {
      return await BluetoothDeviceWindows(remoteId: remoteId).disconnect(
        timeout: timeout,
        queue: queue,
    ...
    try {
      ...
      Ble._autoConnect.remove(remoteId);
      ...
      // invoke
      bool changed = await Ble._invokeMethod('disconnect', remoteId.str);
      ...
      if (!kIsWeb && Platform.isAndroid) {
        ...
        Ble._connectTimestamp.remove(remoteId);
      }
    } finally {
      dtx.give();
      if (queue) {
  ...
  Future<List<BluetoothService>> discoverServices(
    ...
    if (Platform.isWindows) {
      return await BluetoothDeviceWindows(remoteId: remoteId).discoverServices(
          subscribeToServicesChanged: subscribeToServicesChanged,
          timeout: timeout);
    }
    // check connected
    if (isDisconnected) {
      throw BleException(ErrorPlatform.fbp, "discoverServices",
    ...
    try {
      ...
      // invoke
      await Ble._invokeMethod('discoverServices', remoteId.str);
      ...
      // failed?
      if (!response.success) {
        throw BleException(_nativeError, "discoverServices", response.errorCode,
  ...
  DisconnectReason? get disconnectReason {
    if (Platform.isWindows) {
      return BluetoothDeviceWindows(remoteId: remoteId).disconnectReason;
    }
    if (Ble._connectionStates[remoteId] == null) {
      return null;
    }
    int? code = Ble._connectionStates[remoteId]!.disconnectReasonCode;
    String? description =
        Ble._connectionStates[remoteId]!.disconnectReasonString;
  ...
  Stream<BluetoothConnectionState> get connectionState {
    if (Platform.isWindows) {
      return BluetoothDeviceWindows(remoteId: remoteId).connectionState;
    }
    ...
    if (Ble._connectionStates[remoteId] != null) {
      initialValue = _bmToConnectionState(
          Ble._connectionStates[remoteId]!.connectionState);
    }
    return Ble._methodStream.stream
        .where((m) => m.method == "OnConnectionStateChanged")
        .map((m) => m.arguments)
        .map((args) => BmConnectionStateResponse.fromMap(args))
        .where((p) => p.remoteId == remoteId)
  ...
  int get mtuNow {
    ...
    return Ble._mtuValues[remoteId]?.mtu ?? 23;
  }
  ...
  Stream<int> get mtu {
    if (Platform.isWindows) {
      return BluetoothDeviceWindows(remoteId: remoteId).mtu;
    }
    // get initial value from our cache
    int initialValue = Ble._mtuValues[remoteId]?.mtu ?? 23;
    return Ble._methodStream.stream
  ...
  Future<int> readRssi({int timeout = 15}) async {
    if (Platform.isWindows) {
      return await BluetoothDeviceWindows(remoteId: remoteId)
          .readRssi(timeout: timeout);
    }
    // check connected
    if (isDisconnected) {
      throw BleException(ErrorPlatform.fbp, "readRssi",
          FbpErrorCode.deviceIsDisconnected.index, "device is not connected");
    }
    ...
    try {
      ...
      // invoke
      await Ble._invokeMethod('readRssi', remoteId.str);
  ...
  Future<int> requestMtu(int desiredMtu,
    ...
    if (Platform.isWindows) {
      return await BluetoothDeviceWindows(remoteId: remoteId)
          .requestMtu(desiredMtu, predelay: predelay, timeout: timeout);
    }
    // check android
    if (!kIsWeb && Platform.isAndroid == false) {
      throw BleException(ErrorPlatform.fbp, "requestMtu",
    ...
    try {
      ...
      // invoke
      await Ble._invokeMethod('requestMtu', request.toMap());
  ...
  Future<void> requestConnectionPriority(
    ...
    // check android
    if (!kIsWeb && Platform.isAndroid == false) {
      throw BleException(ErrorPlatform.fbp, "requestConnectionPriority",
          FbpErrorCode.androidOnly.index, "android-only");
    }
    ...
    // invoke
    await Ble._invokeMethod('requestConnectionPriority', request.toMap());
  }
  ...
  Future<void> setPreferredPhy({
    ...
    if (Platform.isWindows) {
      return await BluetoothDeviceWindows(remoteId: remoteId)
          .setPreferredPhy(txPhy: txPhy, rxPhy: rxPhy, option: option);
    }
    // check android
    if (!kIsWeb && Platform.isAndroid == false) {
      throw BleException(ErrorPlatform.fbp, "setPreferredPhy",
          FbpErrorCode.androidOnly.index, "android-only");
    }
    ...
    // invoke
    await Ble._invokeMethod('setPreferredPhy', request.toMap());
  }
  ...
  Future<void> createBond({int timeout = 90}) async {
    if (Platform.isWindows) {
      return await BluetoothDeviceWindows(remoteId: remoteId)
          .createBond(timeout: timeout);
    }
    // check android
    if (!kIsWeb && Platform.isAndroid == false) {
      throw BleException(ErrorPlatform.fbp, "createBond",
          FbpErrorCode.androidOnly.index, "android-only");
    }
    ...
    try {
      ...
      // invoke
      bool changed = await Ble._invokeMethod('createBond', remoteId.str);
  ...
  Future<void> removeBond({int timeout = 30}) async {
    if (Platform.isWindows) {
      return await BluetoothDeviceWindows(remoteId: remoteId)
          .removeBond(timeout: timeout);
    }
    // check android
    if (!kIsWeb && Platform.isAndroid == false) {
      throw BleException(ErrorPlatform.fbp, "removeBond",
          FbpErrorCode.androidOnly.index, "android-only");
    }
    ...
    try {
      ...
      // invoke
      bool changed = await Ble._invokeMethod('removeBond', remoteId.str);
  ...
  Future<void> clearGattCache() async {
    if (Platform.isWindows) {
      return await BluetoothDeviceWindows(remoteId: remoteId).clearGattCache();
    }
    // check android
    if (!kIsWeb && Platform.isAndroid == false) {
      throw BleException(ErrorPlatform.fbp, "clearGattCache",
          FbpErrorCode.androidOnly.index, "android-only");
    }
    ...
    // invoke
    await Ble._invokeMethod('clearGattCache', remoteId.str);
  }

  /// Get the current bondState of the device (Android Only)
  Stream<BluetoothBondState> get bondState async* {
    if (Platform.isWindows) {
      // Windows doesn't have a specific bondState stream yet in BluetoothDeviceWindows,
      // but let's route it anyway for consistency if it's ever added.
      // Currently BluetoothDeviceWindows doesn't override this, so it will fall back to base if we don't route.
      // Wait, BluetoothDeviceWindows DOES NOT have bondState.
      // Let's just return a simple stream or route to a dummy if needed.
    }
    // check android
    if (!kIsWeb && Platform.isAndroid == false) {
      throw BleException(ErrorPlatform.fbp, "bondState",
          FbpErrorCode.androidOnly.index, "android-only");
    }

    // get current state if needed
    if (Ble._bondStates[remoteId] == null) {
      var val = await Ble._methodChannel
      ...
      if (Ble._bondStates[remoteId] == null) {
        Ble._bondStates[remoteId] = val;
      }
    }

    yield* Ble._methodStream.stream
        .where((m) => m.method == "OnBondStateChanged")
        .map((m) => m.arguments)
        .map((args) => BmBondStateResponse.fromMap(args))
        .where((p) => p.remoteId == remoteId)
        ...
        .newStreamWithInitialValue(
            _bmToBondState(Ble._bondStates[remoteId]!.bondState));
  }

  /// Get the previous bondState of the device (Android Only)
  BluetoothBondState? get prevBondState {
    var b = Ble._bondStates[remoteId]?.prevState;
  ...
  Future<void> _ensureAndroidDisconnectionDelay(int androidDelay) async {
    if (!kIsWeb && Platform.isAndroid) {
      if (Ble._connectTimestamp.containsKey(remoteId)) {
        Duration minGap = Duration(milliseconds: androidDelay);
        Duration elapsed =
            DateTime.now().difference(Ble._connectTimestamp[remoteId]!);
  ...
  String toString() {
    return 'BluetoothDevice{'
        ...
        'services: ${Ble._knownServices[remoteId]}'
        '}';
  }
