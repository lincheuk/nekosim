...
class BleWindows {
  static void registerWith() {
    // This is called by the generated_plugin_registrant.dart on Windows
    _initialize();
  }
  ...
  static _StreamControllerReEmit<bool> get _isScanning => Ble._isScanning;

  // we use the main Ble static fields for these
  static Map<DeviceIdentifier, String> get _platformNames => Ble._platformNames;
  static Map<DeviceIdentifier, String> get _advNames => Ble._advNames;
  static final _rssiMap = <DeviceIdentifier, int?>{};
  static final _knownServices =
      <DeviceIdentifier, List<BluetoothServiceWindows>>{};
  static Map<DeviceIdentifier, Map<String, List<int>>> get _lastChrs =>
      Ble._lastChrs;
  static final _isNotifying = <DeviceIdentifier, Map<String, bool>>{};
  static final Map<DeviceIdentifier, Map<String, List<BluetoothCharacteristic>>>
      _characteristicCache = {};
  static Map<DeviceIdentifier, List<StreamSubscription>>
      get _deviceSubscriptions => Ble._deviceSubscriptions;
  static Map<DeviceIdentifier, List<StreamSubscription>>
      get _delayedSubscriptions => Ble._delayedSubscriptions;
  static final List<StreamSubscription> _scanSubscriptions = [];

  // stream used for the scanResults public api
  static _StreamControllerReEmit<List<ScanResult>> get _scanResultsList =>
      Ble._scanResults;
  ...
  static final _connectionStream =
      _StreamControllerReEmit(initialValue: <String, bool>{});
  ...
  static Future<void> _initialize() async {
    ...
    WinBle.connectionStream.listen(
      (event) {
        ...
        Ble._connectionStates[DeviceIdentifier(event['device'])] =
            BmConnectionStateResponse(
          remoteId: DeviceIdentifier(event['device']),
          connectionState: event['connected']
  ...
  static Stream<BluetoothAdapterState> get adapterState async* {
    ...
    yield* WinBle.bleState.asBroadcastStream().map(
      (s) {
        ...
        Ble._adapterStateNow = _bmFromAdapterState(_state);
        return _state;
      },
    );
  }
    ...
    _scanSubscription = outputStream.listen(
      (BleDevice? winBleDevice) {
        ...
        if (winBleDevice == null) {
          ...
          BleWindows._platformNames[remoteId] = deviceName;
          BleWindows._advNames[remoteId] = deviceName;
          BleWindows._rssiMap[remoteId] = rssi;
