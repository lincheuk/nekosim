...
class BluetoothCharacteristicWindows extends BluetoothCharacteristic {
  ...
  BluetoothDevice get device => BleWindows.connectedDevices
  ...
  List<int> get lastValue => BleWindows._lastChrs[remoteId]?[_key] ?? [];
  ...
  Stream<List<int>> get lastValueStream => _mergeStreams(
        [
          ...
          BleWindows._charReadWriteStream
              .where((e) => e.$1 == _key)
              .map((e) => e.$2)
        ],
      )
  ...
  Stream<List<int>> get onValueReceived => _mergeStreams(
        [
          ...
          BleWindows._charReadStream.where((e) => e.$1 == _key).map((e) => e.$2)
        ],
      ).map((p) => <int>[...p]).asBroadcastStream();

  // TODO: need to verify
  bool get isNotifying => BleWindows._isNotifying[remoteId]?[_key] ?? false;
  ...
  Future<List<int>> read({int timeout = 15}) async {
    ...
    BleWindows._charReadWriteStreamController.add((_key, value));
    BleWindows._charReadStreamController.add((_key, value));
    BleWindows._lastChrs[remoteId] ??= {};
    BleWindows._lastChrs[remoteId]?[_key] = value;
    return value;
  }
  ...
  Future<bool> setNotifyValue(
    ...
    try {
      ...
      BleWindows._isNotifying[remoteId] ??= {};
      BleWindows._isNotifying[remoteId]?[_key] = notify;
    } catch (e) {
      log(e.toString());
    }
    return true;
