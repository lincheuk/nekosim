...
class BluetoothDeviceWindows extends BluetoothDevice {
  ...
  final _isDiscoveringServices = _StreamControllerReEmit(initialValue: false);
  ...
  String get platformName => BleWindows._platformNames[remoteId] ?? "";
  ...
  String get advName => BleWindows._advNames[remoteId] ?? "";
  ...
  List<BluetoothServiceWindows> get servicesList =>
      BleWindows._knownServices[remoteId] ?? [];
  ...
  Stream<List<BluetoothService>> get servicesStream {
    if (BleWindows._knownServices[remoteId] != null) {
  ...
  void cancelWhenDisconnected(StreamSubscription subscription,
    ...
    if (isConnected == false && next == false) {
      ...
      BleWindows._delayedSubscriptions[remoteId] ??= [];
      BleWindows._delayedSubscriptions[remoteId]!.add(subscription);
    } else {
      BleWindows._deviceSubscriptions[remoteId] ??= [];
      BleWindows._deviceSubscriptions[remoteId]!.add(subscription);
    }
  }

  /// Returns true if this device is currently connected to your app
  bool get isConnected {
    return BleWindows.connectedDevices.contains(this);
  }
  ...
  Future<void> connect({
    ...
    try {
      ...
      BleWindows._deviceSet.add(this);
    } catch (e) {
      log(e.toString());
    }
  }
  ...
  Future<void> disconnect({
    ...
    try {
      ...
      BleWindows._deviceSet.remove(this);

      BleWindows._deviceSubscriptions[remoteId]?.forEach((s) => s.cancel());
      BleWindows._deviceSubscriptions.remove(remoteId);
      // use delayed to update the stream before we cancel it
      Future.delayed(Duration.zero).then((_) {
        BleWindows._delayedSubscriptions[remoteId]?.forEach((s) => s.cancel());
        BleWindows._delayedSubscriptions.remove(remoteId);
      });

      BleWindows._lastChrs[remoteId]?.clear();
      BleWindows._isNotifying[remoteId]?.clear();
    }
  }
  ...
  Future<List<BluetoothService>> discoverServices({
    ...
    List<BluetoothServiceWindows> result =
        List.from(BleWindows._knownServices[remoteId] ?? []);

    try {
      _isDiscoveringServices.add(true);

      final response = await WinBle.discoverServices(_address);
      BleWindows._characteristicCache[remoteId] ??=
      ...
      for (final serviceId in response) {
        ...
        BleWindows._characteristicCache[remoteId] ??= {};
        BleWindows._characteristicCache[remoteId]?[serviceId] ??= [
          ...characteristic.map(
      ...
      result = [
        ...response.map(
          (p) => BluetoothServiceWindows(
            ...
            isPrimary: true,
            // TODO: implementation missing
            characteristics: BleWindows._characteristicCache[remoteId]![p]!,
            // TODO: implementation missing
            includedServices: [],
          ),
        )
      ...
      BleWindows._knownServices[remoteId] = result;

      // sync with central state
      Ble._knownServices[remoteId] = BmDiscoverServicesResult(
        remoteId: remoteId,
        services: result
            .map((s) => BmBluetoothService(
                  remoteId: remoteId,
                  ...
                  characteristics: s.characteristics
                      .map((c) => BmBluetoothCharacteristic(
                            ...
                            properties: BmCharacteristicProperties(
                              broadcast: (c as BluetoothCharacteristicWindows)
                                      .propertiesWinBle
                                      .broadcast ??
                                  false,
                              read: c.propertiesWinBle.read ?? false,
                              writeWithoutResponse:
                                  c.propertiesWinBle.writeWithoutResponse ??
                                      false,
                              write: c.propertiesWinBle.write ?? false,
                              notify: c.propertiesWinBle.notify ?? false,
                              indicate: c.propertiesWinBle.indicate ?? false,
                              authenticatedSignedWrites: c.propertiesWinBle
  ...
  Stream<BluetoothConnectionState> get connectionState async* {
    await BleWindows._initialize();

    final map = BleWindows._connectionStream.latestValue;
  ...
  Future<int> readRssi({int timeout = 15}) async {
    return BleWindows._rssiMap[remoteId] ?? -100;
  }
  ...
  String toString() {
    return 'BluetoothDevice{'
        ...
        'services: ${BleWindows._knownServices[remoteId]}'
        '}';
  }
