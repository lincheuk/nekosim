...
extension _Wrapper on PcscLib {
  ...
  String _getAtrHex(Pointer<UnsignedChar> pbAtr, int atrLength) {
    final bytes = <int>[];
  ...
  Map<String, String> listReadersWithAtr(int hContext) {
    final result = <String, String>{};
    return using((alloc) {
      final pcchReaders = alloc<DWORD>();
      _okOrThrow(SCardListReaders(hContext, nullptr, nullptr, pcchReaders));

      if (pcchReaders.value == 0) return result; // No readers available.

      final mszReaders = alloc<Char>(pcchReaders.value);
      _okOrThrow(SCardListReaders(hContext, nullptr, mszReaders, pcchReaders));

      final readers = multiStringToDart(mszReaders.cast<Utf8>()).toList();

      for (var reader in readers) {
        // print("reader: $reader");
        result[reader] = "000000";
      }
  ...
  ({int hCard, Protocol activeProtocol, String atr}) connect(
    int hContext,
    String reader,
    ShareMode mode,
    Protocol protocol,
    ...
    return using((alloc) {
      ...
      final pcchReaders = alloc<DWORD>();
      final pcbAtrLen = alloc<DWORD>();

      print("reader: $reader alloc");
      ...
      final pbAtr = alloc<Uint8>(pcbAtrLen.value);
      final pbReaderName = alloc<Char>(pcchReaders.value);
      ...
      SCardStatus(
        ...
        nullptr,
        nullptr,
        pbAtr.cast<UnsignedChar>(),
        pcbAtrLen,
      );

      var atr = pbAtr
          .asTypedList(pcbAtrLen.value)
          .map((e) => e.toRadixString(16).padLeft(2, '0'))
          .join('');
      print("Initial ATR (raw): ${atr}");
      return (hCard: phCard.value, activeProtocol: activeProtocol, atr: atr);
    });
  }
  ...
  Uint8List transmit(
    ...
    return using((alloc) {
      ...
      return Uint8List.fromList(
        pbRecvBuffer.asTypedList(pcbRecvLength.value),
      );
    });
  }
...
Future<Map<String, String>> listReadersWithAtr(int hContext) =>
    Isolate.run(() => _pcscLib.listReadersWithAtr(hContext));
...
Future<({int hCard, Protocol activeProtocol, String atr})> connect(
